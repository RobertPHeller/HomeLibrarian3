...
#	puts stderr "*** $type GetInstance: $object does not exist"
    ...
    option -side -readonly yes -default top -validatemethod CheckSide
    method CheckSide {option value} {
      if {[lsearch -exact {top bottom} $value] < 0} {
	error "Not a valid value for $option: $value, should be one of {top bottom}"
      }
      return $value
    }
